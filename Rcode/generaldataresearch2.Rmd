...
KuwaitWeather <- read_csv("Data/2017-2020KuwaitWeather.csv")
#if we want to adjust the weather or add anything to it we can do that here

#making the date in a POSIXct format
KuwaitWeather$date <- as.POSIXct(KuwaitWeather$date, format = '%m/%d/%Y %H:%M') 
...
is.na(KuwaitWeather$tempc) <- startsWith(KuwaitWeather$tempc, "M")
is.na(KuwaitWeather$rh) <- startsWith(KuwaitWeather$rh, "M")

KuwaitWeather$rh <- as.numeric(as.character(KuwaitWeather$rh))
KuwaitWeather$tempc <- as.numeric(as.character(KuwaitWeather$tempc))
...
KuwaitNO2fullt=full_join(KuwaitNO2Trop, KuwaitNO2,by=c("date","Year", "Month", "Day", "YearName", "Month_Name" ))
